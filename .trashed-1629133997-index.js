...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Mais um(a) corno(a) saiu do grupo kkk @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'Calma ai que eu tô trabalhando...',
				success: 'Pronto porra 🤩',
				error: {
					stick: 'Saporra deu erro 😬',
					Iv: 'Link falhou ó'
				},
				only: {
					group: '❌ Tenta usar esse comando em um grupo, vai dar certo❌',
					ownerG: 'Esse comando só pode ser usado pelos ADM, infelizmente vc n é',
					ownerB: '❌ Esse comando só pode ser usado pelo gostoso do Hidan❌',
					admin: 'Foi mal, comando proprio pro Hidan',
					Badmin: 'Me dá ADM nessa porra q eu faço isso'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["558898597666@s.whatsapp.net"] // COLOQUE SEU NÚMERO AQUI SEM ESPAÇOS
...
//AQUI NAO PRECISA MEXER
	...
	//AQUI NAO PRECISA MUDAR
				...
				case 'nsfwloli':
					...
					break*/
				case 'hilih':
					if (args.length < 1) return reply('Cadê a porra do texto???')
					anu = await fetchJson(`https://mhankbarbars.herokuapp.com/api/hilih?teks=${body.slice(7)}`, {method: 'get'})
					reply(anu.result)
					break
		
				case 'yt2mp3':
					if (args.length < 1) return reply('E o link parça')
				...
				case 'ytsearch':
					if (args.length < 1) return reply('Qual video procuras?')
				...
				case 'stalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'Qual a pessoa vc quer stalkear?', text, {quoted: mek})
				...
				case 'url2img':
					...
					if (args.length < 2) return reply('Link?')
	...
	//AQUI NAO PRECISA MUDAR
			...
			case 'tagall':
				...
				case 'clearall':
					if (!isOwner) return reply('Apagar todos os chats pra quem?') 
					anu = await client.chats.all()
					client.setMaxListeners(25)
					for (let _ of anu) {
						client.deleteChat(_.jid)
					}
					reply('Seu chat foi apagado')
					break
				...
				case 'bc':
					...
					if (mentioned.length > 1) {
						teks = 'Infeliz promovido á ADM com sucesso \n'
				...
				case 'demote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Menos um(a) corno(a) como ADM \n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('Quem é que tu quer q eu adicione?') 
					if (args[0].startsWith('08')) return reply('Escreve o numero do jeito certo ai nmrl')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						client.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('N deu pra adicionar não, numero dele(a) é privado' )
					}
					break
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Vai expulsar quem?')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Um ban gostoso em: \n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Ban feito, tirei essa miséra: @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
//AQUI NAO PRECISA MUDAR		
...
//AQUI NAO PRECISA MUDAR                   
				...
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ Marca a figurinha ai❌')
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('Deu errado')
...
//NAO FUNCIONA MAIS	
				...
				case 'welcome':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Membro novo tem q mamar os ADM')
						welkom.push(from)
